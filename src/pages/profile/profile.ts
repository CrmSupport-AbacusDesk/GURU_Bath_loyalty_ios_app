...
export class ProfilePage {
  ...
  uploadImage(profile)
  {
    ...
    ShareApp()
    {
        if((this.karigar_detail.type == "Plumber" || this.karigar_detail.type == "Dealer") && this.karigar_detail.referral_code!="")
        {
            this.ref_code = ' and use my Code *'+this.karigar_detail.referral_code+'* to get points back in your wallet'
        }
        this.socialSharing.share('Hey There ! here is an awesome app from B L and Company  ..Give it a try https://play.google.com/store/apps/details?id=com.guru_bath.app'+this.ref_code).then(() => {
    }).catch((e) => {
        console.log(e);
    });
    }
